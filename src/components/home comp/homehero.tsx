      ...
      <div ref={heroRef} className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8">
            {/* Trust Badge */}
            <div ref={trustBadgeRef} className="flex items-center gap-2 surface-card backdrop-blur rounded-full px-4 py-2 shadow-sm border border-border w-fit transition-colors duration-300">
              ...
              <span className="text-sm font-medium theme-text-secondary">Trusted by clients & freelancers worldwide</span>
            </div>

            {/* Main Headline */}
            <div className="space-y-4">
              <h1 ref={headlineRef} className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight theme-text-primary transition-colors duration-300">
                Where
              ...
              <p ref={subtitleRef} className="text-xl theme-text-secondary leading-relaxed max-w-2xl transition-colors duration-300">
                Whether you&apos;re hiring top talent or showcasing your skills, SkillConnect connects the right people for every project. Join millions of freelancers and clients worldwide.
              </p>
            </div>

            {/* Popular Searches */}
            <div ref={popularSectionRef} className="space-y-3">
              <p className="text-sm theme-text-secondary font-medium transition-colors duration-300">Popular for clients:</p>
              <div className="flex flex-wrap gap-2">
                {["Web Development", "Logo Design", "Content Writing", "Mobile Apps", "SEO"].map((tag) => (
                  <button key={tag} className="chip" data-variant="primary">
                    {tag}
                  </button>
                ))}
              </div>
              <p className="text-sm theme-text-secondary font-medium mt-4 transition-colors duration-300">Popular for freelancers:</p>
              <div className="flex flex-wrap gap-2">
                {["Remote Jobs", "Part-time Work", "Project Based", "Long-term", "Entry Level"].map((tag) => (
                  <button key={tag} className="chip" data-variant="secondary">
                    {tag}
                  </button>
                ))}
              </div>
            ...
            <div ref={ctaButtonsRef} className="flex flex-col sm:flex-row gap-4 pt-4">
              <Link href="/select">
                <Button className="cursor-pointer bg-primary text-primary-foreground hover:bg-primary/90 px-8 py-4 rounded-xl font-semibold shadow-md hover:shadow-lg transition-all duration-200 text-lg group">
              ...
              <Link href="/select">
                <Button className="cursor-pointer bg-accent text-accent-foreground hover:bg-accent/90 px-8 py-4 rounded-xl font-semibold shadow-md hover:shadow-lg transition-all duration-200 text-lg group">
          ...
          <div className="relative">
            ...
            <div ref={floatingCard1Ref} className="hidden lg:block absolute -top-6 -left-4 lg:-left-6 surface-card backdrop-blur rounded-2xl shadow-lg p-4 border border-border transition-colors duration-300">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center transition-colors duration-300 bg-muted">
                  <CheckCircle className="w-6 h-6 text-foreground transition-colors duration-300" />
                </div>
                <div>
                  <p className="font-bold theme-text-primary transition-colors duration-300">Verified</p>
                  <p className="text-sm theme-text-secondary transition-colors duration-300">Talent Profiles</p>
                </div>
              </div>
            </div>

            <div ref={floatingCard2Ref} className="hidden lg:block absolute -bottom-6 -right-4 lg:-right-6 surface-card backdrop-blur rounded-2xl shadow-lg p-4 border border-border transition-colors duration-300">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center transition-colors duration-300 bg-muted">
                  <Star className="w-6 h-6 text-foreground transition-colors duration-300" />
                </div>
                <div>
                  <p className="font-bold theme-text-primary transition-colors duration-300">Based on</p>
                  <p className="text-sm theme-text-secondary transition-colors duration-300">Reviews & Ratings</p>
                </div>
              </div>
            </div>

            <div ref={floatingCard3Ref} className="hidden lg:block absolute top-1/2 -left-6 lg:-left-8 -translate-y-1/2 transform surface-card backdrop-blur rounded-2xl shadow-lg p-4 border border-border transition-colors duration-300">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center transition-colors duration-300 bg-muted">
                  <Users className="w-6 h-6 text-foreground transition-colors duration-300" />
                </div>
                <div>
                  <p className="font-bold theme-text-primary transition-colors duration-300">2K+</p>
                  <p className="text-sm theme-text-secondary transition-colors duration-300">Active Community Users</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div ref={statsRef} className="grid grid-cols-2 lg:grid-cols-4 gap-8 text-center">
            {[
              ...
              <div key={index} className="space-y-2">
                <p className="text-3xl font-bold theme-text-primary transition-colors duration-300">{stat.value}</p>
                <p className="theme-text-secondary font-medium transition-colors duration-300">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
